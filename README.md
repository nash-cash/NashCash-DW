[![NPM](https://nodei.co/npm/NashCashd-ha.png?downloads=true&stars=true)](https://nodei.co/npm/NashCashd-ha/)

[![Build Status](https://travis-ci.org/brandonlehmann/NashCashd-ha.png?branch=master)](https://travis-ci.org/brandonlehmann/NashCashd-ha) [![Build Status](https://ci.appveyor.com/api/projects/status/github/brandonlehmann/NashCashd-ha?branch=master&svg=true)](https://ci.appveyor.com/project/brandonlehmann/NashCashd-ha/branch/master)

# NashCashd High-Availability Daemon Wrapper

This project is designed to wrap the NashCashd daemon on a *nix system and monitor it for hangups, locks, fork, or other events that cause the daemon to stop responding to requests in an accurate manner.
...
5. [Documentation](#documentation)
   1. [Methods](#methods)
   2. [Events](#events)
   3. [NashCashd RPC API Interface](#NashCashd-rpc-api-interface)
...
* [NashCashd](https://github.com/turtlecoin/turtlecoin/releases) v0.19.0 or higher

## Easy Start

You *must* copy ```NashCashd``` into the ```NashCashd-ha``` folder for the easy start process to occur.

```bash
git clone https://github.com/turtlecoin/NashCashd-ha.git
cd NashCashd-ha
cp <NashCashd> .
...
pm2 start service.js --name NashCashd
pm2 save
```
...
Practically all NashCashd command line arguments are exposed in the constructor method. Simply include them in your list of options to get activate or use them. Default values are defined below.

```javascript
var daemon = new NashCashd({
  // These are our NashCashd-ha options
  ...
  // These are the standard NashCashd options
  path: './NashCashd', // Where can I find NashCashd?
...
This event is emitted when the daemon starts. The callback contains the command line arguments supplied to NashCashd.
